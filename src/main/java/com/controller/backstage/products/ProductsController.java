...
public class ProductsController {
	...
	public String ProductsInsert(HttpServletRequest request, Products products, MultipartFile file) {
		...
		if (file != null) {
			/*
			 * String
			 * uploadPath=request.getServletContext().getRealPath("files");
			 * //下载到的文件夹位置 String fileName = file.getOriginalFilename();
			 * //原始文件名称 String path = uploadPath+File.separator+fileName;
			 * //文件完整路径名称 File newFile = new File(path);
			 * if(!newFile.getParentFile().exists()){ //判断存储文件的路径是否存在，如果不存在则创建
			 * newFile.getParentFile().mkdirs(); } try {
			 * file.transferTo(newFile); } catch (IllegalStateException e) { //
			 * TODO Auto-generated catch block e.printStackTrace(); } catch
			 * (IOException e) { // TODO Auto-generated catch block
			 * e.printStackTrace(); }
			 */
			productsadd.setProducturl(UploadImage.addImage(file, "/backstage/product", request));
	...
	public String UploadFilestest(HttpServletRequest request, MultipartFile[] graphTheorise, int id) {
		 ...
		 * //文件下载存储的路径 for(MultipartFile files:graphTheorise){ //遍历文件 String
		 * fileName; //文件名 try{ fileName = new
		 * String(files.getOriginalFilename().getBytes("iso-8859-1"), "utf-8");
		 * String newFileName=UUID.randomUUID()+fileName; String
		 * endPath=uploadPath+File.separator+newFileName; File loadFile=new
		 * File(endPath); //新建文件 if (!loadFile.getParentFile().exists()) {
		 * loadFile.getParentFile().mkdirs(); } files.transferTo(loadFile);
		 * list.add(endPath); }catch(Exception e){ e.printStackTrace(); } }
		 */
		if (graphTheorise != null & graphTheorise[0].getName().length() > 0) {
			list = UploadImage.addImages(graphTheorise, "/backstage/product/others", request);
		}
		if (list != null) {
	...
	public String updateProducts(HttpServletRequest request, MultipartFile[] graphTheorise, int id) {
		 ...
		 */
		if (graphTheorise != null & graphTheorise[0].getName().length() > 0) {
			List<Showsphotos> showsList = showsphotosService.selectBypid(pid);
			if (showsList != null & showsList.size() > 0) {
				List<String> showUrl = new ArrayList<String>();
				for (Showsphotos shows : showsList) {
					showUrl.add(shows.getImage());
				}
				UploadImage.deleteFile(UploadImage.getOSSClient(), showUrl);
			}
			list = UploadImage.addImages(graphTheorise, "/backstage/product/others", request);
		}
		if (showphotos != null && showphotos.size() > 0) {
			int size = list.size();
			String[] picuter = (String[]) list.toArray(new String[size]); // picuter[size]
																			// 传入的图片
	...
	public String QueryBypnameProducts(HttpServletRequest request, HttpServletResponse response, String pname) {
	...
	public String UpdateProducts(HttpServletRequest request, Products products, MultipartFile file) {
		if (file != null) {
			...
			Products prd = productService.selectByPrimaryKey(products.getId());
			String urls = prd.getProducturl();
			UploadImage.deleteFile(UploadImage.getOSSClient(), urls);
			products.setProducturl(UploadImage.addImage(file, "/backstage/product", request));
		}
	...
	public String searchInvList(Page page, HttpServletRequest request) throws UnsupportedEncodingException { // 展示商品
																												// 分页实现
