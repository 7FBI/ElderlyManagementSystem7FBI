...
public class ShopPrices {
	...
	// 这个订单的商品哪些存在打折价
	public static Double getAllShowPrices(String oid, DiscountService discountService,
	...
	// 订单重置价格(存在打折)
	public static List<Orderdetails> getAllOrderdetailsPrices(String oid, DiscountService discountService,
			OrderdetailsService orderdetailsService) {
		List<Orderdetails> list = orderdetailsService.selectByOrdersId(oid);
		int i = 0;
		for (Orderdetails orderdetails : list) {
			...
			// 存在打折则，重新赋值
			if (discount != null) {
				double d = d2 * discount.getDiscountprice();
	...
	public static List<Orderdetails> getAllOrderdetailsPrices(String oid, ProductsService productsService,
	...
	public static Double getNewMoney(Integer pid, ProductsService productsService,
			GroupbuyingService groupbuyingService, DiscountService discountService) {
		Double moneys = 0.0;
	...
	// 仅仅获取数据
	public static Double getAllShowPrices(String oid, OrdersService ordersService,
			OrderdetailsService orderdetailsService) {
		List<Orderdetails> list = orderdetailsService.selectByOrdersId(oid);
		double d = 0.0;
		Groupbuying g = new Groupbuying();
