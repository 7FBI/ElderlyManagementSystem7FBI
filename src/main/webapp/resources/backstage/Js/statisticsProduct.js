...
function getValuesNameSumNum(data) {
	...
	for ( var k in data) {
		var items = {
			name : data[k].pname.toString(),
			value : parseInt(data[k].sumNum)
		};
		mapdata[k] = items;
	}
	return mapdata;
...
function getValuesNameSumMoney(data) {
	...
	for ( var k in data) {
		var items = {
			name : data[k].pname.toString(),
			value : parseInt(data[k].sumMoney)
		};
		mapdata[k] = items;
	}
	return mapdata;
...
function getValuesNameSumNum2(xnames, data) {
	...
	for ( var k in data) {
		var items = {
			name : xnames[k].toString(),
			value : parseInt(data[k])
		};
		mapdata[k] = items;
	}
	return mapdata;
...
function setTypeOptionThree(xTy, titles, legends, xnames, numbers, documentsId,
		col) {
	var myChart = echarts.init(document.getElementById(documentsId));
	var option = {
		color : col,
